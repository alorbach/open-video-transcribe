# Screenshots Directory

This directory contains screenshots for the user documentation.

## Current Screenshots

The following screenshots are currently used in the documentation:

1. **setup-script.png** - Screenshot of the setup script running (showing Python version selection)
2. **main-window.png** - Main application window with all controls visible

## Screenshot Guidelines

- **Format**: PNG format recommended
- **Size**: 800-1200 pixels wide is ideal
- **Quality**: Clear, readable text and UI elements
- **Content**: Show relevant UI elements clearly
- **Privacy**: Ensure no personal information is visible

## How to Take Screenshots

### Windows
- Use Snipping Tool or Win+Shift+S
- Capture the relevant window/dialog
- Save as PNG

### Linux
- Use Screenshot tool (usually in Applications menu)
- Or use: `gnome-screenshot -w` (GNOME) or `spectacle` (KDE)

### macOS
- Use Cmd+Shift+4 for area selection
- Or Cmd+Shift+3 for full screen
- Save as PNG

## Screenshot Files

- `setup-script.png` - Shows the setup.bat script running with Python version selection
- `main-window.png` - Shows the main application window with model selection, file selection, and settings

These screenshots are referenced in `USER_GUIDE.md` and `INSTALLATION.md`.

